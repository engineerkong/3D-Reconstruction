class KDTree(object):
    ...
    def __init__(self, points, dim, dist_sq_func=None):
        ...
        def get_dnn(node, point, n, d, return_dist_sq, heap, i=0, tiebreaker=1):
            if node is not None:
                dist_sq = dist_sq_func(point, node[2])
                dx = node[2][i] - point[i]
                if len(heap) < n:
                ...
                # Goes into the left branch, then the right branch if needed
                for b in (dx < 0, dx >= 0)[:1 + (dx * dx < -heap[0][0])]:
                    get_dnn(node[b], point, n, d, return_dist_sq,
                        heap, i, (tiebreaker << 1) | b)
            if tiebreaker == 1:
                result = []
                for h in sorted(heap):
                    if -h[0] < d:
                        result.append(h[2])
                return result[::-1]
                # return [h[2] if -h[0] < d else continue
                #     for h in sorted(heap)][::-1]
    ...
    def get_dnn(self, point, n, d, return_dist_sq=True):
        """Returns k nearest neighbors.
        ...
        list<array-like>
            ...
            If `return_dist_sq` is true, the return will be:
                ...
                [point, ...]
        """
        return self._get_dnn(self._root, point, n, d, return_dist_sq, [])
